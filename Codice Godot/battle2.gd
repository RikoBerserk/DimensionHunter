...
func _ready():
	...
	var level = randi_range(4,8)
	var health = randi_range(25,50)
	
	exp = randi_range(15,25)
	$Enemy.get_child(0).Level = level
	$Enemy.get_child(0).Health = randi_range(33, 36)
...
func MonsterTurn(sceltaPlayer, damagePlayer, attack):
	...
	var damage = 1
	if $Enemy.get_child(0).Level == 4:
		damage = randi_range(10,15)
	elif $Enemy.get_child(0).Level == 5:
		damage = randi_range(14,18)
	elif $Enemy.get_child(0).Level == 6:
		damage = randi_range(16,20)
	elif $Enemy.get_child(0).Level == 7:
		damage = randi_range(18,22)
	elif $Enemy.get_child(0).Level == 8:
		damage = randi_range(21,23)
	elif $Enemy.get_child(0).Level == 9:
		damage = randi_range(22,25)

	
	if sceltaEnemy == 0: #inizia nuovamente il ciclo
		sceltaEnemy = 3
